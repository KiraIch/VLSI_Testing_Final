...
// =========================================================
// 5. Ultimate Multi-Fault Engine (Cone Pruning & Deduplication)
// =========================================================
void run_diag(const string& ptn_path, const string& ckt_path, const string& log_path) {
    ...
    // 【優化 2：拓樸圓錐修剪 (Safe Cone Pruning)】 大幅降低 Runtime
    vector<bool> in_fail_cone(ckt.num_wires, false);
    queue<int> q;
    for (size_t i = 0; i < ckt.pos.size(); ++i) {
        if (po_ever_failed[i]) {
            int po_wire = ckt.pos[i];
            in_fail_cone[po_wire] = true; q.push(po_wire);
        }
    }
    vector<vector<int>> rev_adj(ckt.num_wires);
    for (int i = 0; i < ckt.num_wires; ++i) {
        for (int fi : ckt.gates[i].fanins) rev_adj[i].push_back(fi);
    }
    while(!q.empty()) {
        int u = q.front(); q.pop();
        for (int fi : rev_adj[u]) {
            if (!in_fail_cone[fi]) { in_fail_cone[fi] = true; q.push(fi); }
        }
    }
    ...
    // GI 錯誤 (【優化 3】：剃除 1-Fanin 無效模擬)
    for (int gid = 0; gid < ckt.num_wires; ++gid) {
        const auto& g = ckt.gates[gid];
        if (g.type == "PO" || g.type == "PI" || g.type == "NOT" || g.type == "BUF") continue; 
    ...
    for (size_t i = 0; i < candidates.size() && output_count < 10; ++i) {
        ...
        // 【優化 1：智能過濾等效線路名稱 (Deduplication)】大幅降低 Resolution 懲罰
        set<string> printed_names;
        printed_names.insert(candidates[i].name);
        vector<string> eq_faults;
        
        for (size_t j = i + 1; j < candidates.size(); ++j) {
            if (candidates[j].res.sig_bits == candidates[i].res.sig_bits) {
